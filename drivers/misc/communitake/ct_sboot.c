...
#define BOOT_PART_NAME  "/dev/block/platform/msm_sdcc.1/by-name/boot"

/* Boot IMG header
 * Taken from system/core/mkbootimg/bootimg.h */
...
static void fp_work_handler(struct work_struct *w);
...
#define FP_WORK_ITEM_INIT(fpwi_name, fwpi_handler, sha1buf, hmacbuf) { \
	.work = __WORK_INITIALIZER(fpwi_name.work, fwpi_handler),  \
	.sha1 = sha1buf,   \
	.hmac = hmacbuf,   \
	.size = 0   \
};
...
static int gen_boot_signatures(u8 *sha1, u8 *hmac)
{
	...
	if (IS_ERR_OR_NULL(fp)) {
		pr_err("Failed to open boot patition. err=%ld\n", PTR_ERR(fp));
        fp = NULL;
		goto Exit;
	}
...
static void fp_work_handler(struct work_struct *w)
{
	struct fp_work_item *fwpi = container_of(w, struct fp_work_item, work);

	fwpi->size = gen_boot_signatures(fwpi->sha1, fwpi->hmac);
}
			     ...
			     char *buf)
{
	...
	u8 hmac[SHA1_SIZE];
	int i;
	struct fp_work_item fwpi = FP_WORK_ITEM_INIT(fwpi, fp_work_handler,
						     sha1, hmac);

	memset(sha1, 0xCC, sizeof(sha1));
	memset(hmac, 0xCC, sizeof(hmac));

	/* Generate signature in kernel context via global workqueue */
	schedule_work(&fwpi.work);
	flush_work(&fwpi.work);
...
int communitake_sysfs_init (void)
{
	...
	if (rc) {
		pr_err("failed to create /sys/kenel/communitake/sboot_fp \n");
		goto Exit;
	}
...
Exit:
    /* We keep our kobj since others rely on it as well */
	return rc;
}
